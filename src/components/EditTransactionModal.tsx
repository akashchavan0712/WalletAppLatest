...
import { useCategories, useUpdateTransaction, useTransactions } from "@/hooks/useTransactions";
import { toast } from "sonner";

export default function EditTransactionModal() {
  const { editTransactionId, setEditTransactionId } = useAppStore();
  ...
  useEffect(() => {
    if (transaction) {
      ...
      const isIncomeCustom = transaction.type === "income" && !["salary", "freelance", "business"].includes(transaction.category.toLowerCase());
      if (isIncomeCustom) {
        setCategory("custom");
        setCustomIncomeCategory(transaction.category);
      } else {
          ...
          >
            <div className="w-full max-w-[480px] glass-card p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-display font-bold text-xl text-foreground">Edit Transaction</h2>
                <button onClick={() => setEditTransactionId(null)} className="text-muted-foreground hover:text-foreground p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
